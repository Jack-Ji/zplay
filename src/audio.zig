...
const miniaudio = zp.deps.miniaudio;
...
pub const Engine = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, opt: EngineOption) !*Engine {
        ...
        const rc = miniaudio.ma_engine_init(&config, &engine.engine);
    ...
    pub fn deinit(e: *Engine) void {
        while (e.sound_list.pop()) |node| {
            _ = miniaudio.ma_sound_uninit(&node.data.sound);
            e.allocator.destroy(node);
        }
        while (e.sound_group_list.pop()) |node| {
            _ = miniaudio.ma_sound_group_uninit(&node.data.group);
            e.allocator.destroy(node);
        }
        _ = miniaudio.ma_engine_uninit(&e.engine);
        e.allocator.destroy(e);
    }

    pub fn start(e: *Engine) void {
        _ = miniaudio.ma_engine_start(&e.engine);
    }

    pub fn stop(e: *Engine) void {
        _ = miniaudio.ma_engine_stop(&e.engine);
    }
    ...
    pub fn getSampleRate(e: *Engine) u32 {
        return @intCast(u32, miniaudio.ma_engine_get_sample_rate(&e.engine));
    }
    ...
    pub fn setListenerPosition(e: *Engine, listener_index: u32, pos_x: f32, pos_y: f32, pos_z: f32) void {
        ...
        _ = miniaudio.ma_engine_listener_set_position(
            &e.engine,
    ...
    pub fn setListenerDirection(e: *Engine, listener_index: u32, dir_x: f32, dir_y: f32, dir_z: f32) void {
        assert(listener_index < e.option.listener_num);
        _ = miniaudio.ma_engine_listener_set_direction(
            &e.engine,
            @intCast(c_uint, listener_index),
            dir_x,
            dir_y,
    ...
    pub fn setListenerWorldUp(e: *Engine, listener_index: u32, dir_x: f32, dir_y: f32, dir_z: f32) void {
        assert(listener_index < e.option.listener_num);
        _ = miniaudio.ma_engine_listener_set_world_up(
            &e.engine,
            @intCast(c_uint, listener_index),
            dir_x,
            dir_y,
    ...
    pub fn setListenerCone(e: *Engine, listener_index: u32, inner_angle: f32, outer_angle: f32, outer_gain: f32) void {
        ...
        assert(outer_gain > 1);
        _ = miniaudio.ma_engine_listener_set_cone(
            &e.engine,
            listener_index,
            inner_angle,
            outer_angle,
    ...
    pub const SoundOption = struct {
        ...
        fn toInt(o: SoundOption) c_uint {
            var flags: c_uint = 0;
            if (o.stream) {
                flags |= miniaudio.MA_SOUND_FLAG_STREAM;
            }
            if (o.async_read) {
                flags |= miniaudio.MA_SOUND_FLAG_ASYNC;
            }
            if (o.decode) {
                flags |= miniaudio.MA_SOUND_FLAG_DECODE;
            }
            if (o.no_spatialization) {
                flags |= miniaudio.MA_SOUND_FLAG_NO_SPATIALIZATION;
            }
            if (o.no_doppler) {
                flags |= miniaudio.MA_SOUND_FLAG_NO_PITCH;
            }
            return flags;
        }
    };
...
pub const Sound = struct {
    ...
    pub fn setVolume(snd: *Sound, volume: f32) void {
        miniaudio.ma_sound_set_volume(&snd.sound, volume);
    }

    pub fn getVolume(snd: *Sound) f32 {
        return miniaudio.ma_sound_get_volume(&snd.sound);
    }

    pub fn setLooping(snd: *Sound, looping: bool) void {
        miniaudio.ma_sound_set_looping(&snd.sound, @as(c_uint, @boolToInt(looping)));
    }
    ...
    pub fn setStartTime(snd: *Sound, t: TimeUnit) void {
        switch (t) {
            .pcm_frames => |frames| {
                miniaudio.ma_sound_set_start_time_in_pcm_frames(&snd.sound, @intCast(c_ulonglong, frames));
            },
            .milliseconds => |ms| {
                miniaudio.ma_sound_set_start_time_in_milliseconds(&snd.sound, @intCast(c_ulonglong, ms));
            },
        }
    }
    ...
    pub fn setStopTime(snd: *Sound, t: TimeUnit) void {
        switch (t) {
            .pcm_frames => |frames| {
                miniaudio.ma_sound_set_stop_time_in_pcm_frames(&snd.sound, @intCast(c_ulonglong, frames));
            },
            .milliseconds => |ms| {
                miniaudio.ma_sound_set_stop_time_in_milliseconds(&snd.sound, @intCast(c_ulonglong, ms));
            },
        }
    }
    ...
    pub fn setFadeIn(snd: *Sound, volume_begin: f32, volume_end: f32, t: TimeUnit) void {
        switch (t) {
            .pcm_frames => |frames| {
                miniaudio.ma_sound_set_fade_in_pcm_frames(&snd.sound, volume_begin, volume_end, @intCast(c_ulonglong, frames));
            },
            .milliseconds => |ms| {
                miniaudio.ma_sound_set_fade_in_milliseconds(&snd.sound, volume_begin, volume_end, @intCast(c_ulonglong, ms));
            },
        }
    }

    pub fn setPosition(snd: *Sound, pos_x: f32, pos_y: f32, pos_z: f32) void {
        miniaudio.ma_sound_set_position(&snd.sound, pos_x, pos_y, pos_z);
    }

    pub fn getPosition(snd: *Sound) miniaudio.ma_vec3f {
        return miniaudio.ma_sound_get_position(&snd.sound);
    }

    pub fn setDirection(snd: *Sound, dir_x: f32, dir_y: f32, dir_z: f32) void {
        miniaudio.ma_sound_set_direction(&snd.sound, dir_x, dir_y, dir_z);
    }

    pub fn getDirection(snd: *Sound) miniaudio.ma_vec3f {
        return miniaudio.ma_sound_get_direction(&snd.sound);
    }

    pub fn setVelocity(snd: *Sound, v_x: f32, v_y: f32, v_z: f32) void {
        miniaudio.ma_sound_set_velocity(&snd.sound, v_x, v_y, v_z);
    }

    pub fn getVelocity(snd: *Sound) miniaudio.ma_vec3f {
...
pub const SoundGroup = struct {
    ...
    pub fn stop(grp: *SoundGroup) void {
        _ = miniaudio.ma_sound_group_stop(&grp.group);
    }

    pub fn setVolume(grp: *SoundGroup, volume: f32) void {
        miniaudio.ma_sound_group_set_volume(&grp.group, volume);
    }

    pub fn getVolume(grp: *SoundGroup) f32 {
        return miniaudio.ma_sound_group_get_volume(&grp.group);
    }

    pub fn setStartTime(grp: *SoundGroup, t: TimeUnit) void {
        switch (t) {
            .pcm_frames => |frames| {
                miniaudio.ma_sound_group_set_start_time_in_pcm_frames(&grp.group, @intCast(c_ulonglong, frames));
            },
            .milliseconds => |ms| {
                miniaudio.ma_sound_group_set_start_time_in_milliseconds(&grp.group, @intCast(c_ulonglong, ms));
            },
        }
    }
    ...
    pub fn setStopTime(grp: *SoundGroup, t: TimeUnit) void {
        switch (t) {
            .pcm_frames => |frames| {
                miniaudio.ma_sound_group_set_stop_time_in_pcm_frames(&grp.group, @intCast(c_ulonglong, frames));
            },
            .milliseconds => |ms| {
                miniaudio.ma_sound_group_set_stop_time_in_milliseconds(&grp.group, @intCast(c_ulonglong, ms));
            },
        }
    }
    ...
    pub fn setFadeIn(grp: *SoundGroup, volume_begin: f32, volume_end: f32, t: TimeUnit) void {
        switch (t) {
            .pcm_frames => |frames| {
                miniaudio.ma_sound_group_set_fade_in_pcm_frames(&grp.group, volume_begin, volume_end, @intCast(c_ulonglong, frames));
            },
            .milliseconds => |ms| {
                miniaudio.ma_sound_group_set_fade_in_milliseconds(&grp.group, volume_begin, volume_end, @intCast(c_ulonglong, ms));
            },
        }
    }

    pub fn setPosition(grp: *SoundGroup, pos_x: f32, pos_y: f32, pos_z: f32) void {
        miniaudio.ma_sound_group_set_position(&grp.group, pos_x, pos_y, pos_z);
    }

    pub fn getPosition(grp: *SoundGroup) miniaudio.ma_vec3f {
        return miniaudio.ma_sound_group_get_position(&grp.group);
    }

    pub fn setDirection(grp: *SoundGroup, dir_x: f32, dir_y: f32, dir_z: f32) void {
        miniaudio.ma_sound_group_set_direction(&grp.group, dir_x, dir_y, dir_z);
    }

    pub fn getDirection(grp: *SoundGroup) miniaudio.ma_vec3f {
        return miniaudio.ma_sound_group_get_direction(&grp.group);
    }

    pub fn setVelocity(grp: *SoundGroup, v_x: f32, v_y: f32, v_z: f32) void {
        miniaudio.ma_sound_group_set_velocity(&grp.group, v_x, v_y, v_z);
    }

    pub fn getVelocity(grp: *SoundGroup) miniaudio.ma_vec3f {
    ...
    pub fn getPitch(grp: *SoundGroup) f32 {
        return miniaudio.ma_sound_group_get_pitch(&grp.group);
    }

    pub fn setCone(grp: *SoundGroup, inner_angle: f32, outer_angle: f32, outer_gain: f32) void {
        assert(inner_angle >= 0 and outer_gain >= 0 and outer_angle >= inner_angle);
        assert(outer_gain > 1);
        _ = miniaudio.ma_sound_group_set_cone(
            &grp.group,
            inner_angle,
            outer_angle,
            outer_gain,
    ...
    pub fn getCone(grp: *SoundGroup, inner_angle: *f32, outer_angle: *f32, outer_gain: *f32) void {
        _ = miniaudio.ma_sound_group_get_cone(
            &grp.group,
            inner_angle,
            outer_angle,
            outer_gain,
